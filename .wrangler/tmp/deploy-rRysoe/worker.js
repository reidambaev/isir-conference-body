...
async function handleApiRequest(request, env, url) {
  ...
  if (url.pathname === "/api/abstract-submission" && request.method === "POST") {
...
async function handleRegistration(request, env, corsHeaders) {
  try {
    ...
    const earlyBirdDeadline = (/* @__PURE__ */ new Date("2026-07-10")).getTime();
    const isEarlyBird = registrationDate < earlyBirdDeadline;
    const ticketPrice = ticketPrices[data.ticketType]?.[isEarlyBird ? "early" : "standard"] || 0;
    const accompanyingPrice = (isEarlyBird ? 250 : 350) * (data.accompanyingPersonCount || 0);
...
__name(handleRegistration, "handleRegistration");
...
__name(handleGetRegistrations, "handleGetRegistrations");
async function handleAbstractSubmission(request, env, corsHeaders) {
  try {
    ...
    const submissionDeadline = (/* @__PURE__ */ new Date("2026-04-30")).getTime();
    const submissionOpens = (/* @__PURE__ */ new Date("2026-01-15")).getTime();
    const now = Date.now();
    ...
    const submissionId = `ABS-${Date.now()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
    const submissionDate = Date.now();
    let authorsData = typeof data.authors === "string" ? JSON.parse(data.authors) : data.authors;
    ...
    await env.ISIR_DB.prepare(
      ...
      data.conflictOfInterest === "yes" ? data.conflictDetails?.trim() || null : null,
    ...
    let affiliationsData = typeof data.affiliations === "string" ? JSON.parse(data.affiliations) : data.affiliations;
...
__name(handleAbstractSubmission, "handleAbstractSubmission");
...
__name(handleVisaRequest, "handleVisaRequest");
export {
