...
- **Oral session speaker letter** (admin): after placing an oral abstract in YI or N1–N5 under **Sessions**, send the session letter manually (`POST /api/admin/abstracts/:id/send-oral-session-notification` or bulk `/api/admin/abstracts/send-oral-session-notifications`). Emails both presenting and corresponding authors. Requires D1 columns from `db/migration_add_oral_session.sql`. YI uses the Young Investigator Award opening.
