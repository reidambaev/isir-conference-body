...
async function handleApiRequest(request, env, url) {
  ...
  if (
    ...
    return handleAdminSeedBundledSpeakerCatalog(request, env, corsHeaders);
  }

  // POST /api/admin/speaker-catalog/import-tsv — tab-separated file (speaker_key, name, affiliation, email)
  if (
    url.pathname === "/api/admin/speaker-catalog/import-tsv" &&
    request.method === "POST"
  ) {
...
function stableCatalogSeedId(speakerKey) {
  return `seed-${String(speakerKey).replace(/[^a-zA-Z0-9_-]/g, "-")}`;
}

async function upsertCatalogSpeakerRow(
  env,
  {
    speakerKey,
    displayName,
  ...
  const em =
    email != null && String(email).trim()
      ? String(email).trim()
      : catalogSeedPlaceholderEmail(speakerKey);
  const pos =
...
async function handleAdminSeedBundledSpeakerCatalog(request, env, corsHeaders) {
  const auth = ensureAdmin(request, env, corsHeaders);
  if (auth) return auth;
  if (!env.ISIR_DB) {
    return jsonResponse(
  ...
  try {
    ...
    console.error("handleAdminSeedBundledSpeakerCatalog:", e);
    return jsonResponse(
      {
        success: false,
        error: e?.message || "Catalog seed failed",
      },
      500,
      corsHeaders,
    );
...
async function handleAdminImportSpeakerCatalogTsv(request, env, corsHeaders) {
  const auth = ensureAdmin(request, env, corsHeaders);
  if (auth) return auth;
  if (!env.ISIR_DB) {
    return jsonResponse(
  ...
  try {
    ...
    if (!file || typeof file.text !== "function") {
      return jsonResponse(
        { success: false, error: "Missing file field (TSV)" },
        400,
        corsHeaders,
      );
    }
    ...
    if (lines.length < 2) {
      return jsonResponse(
        { success: false, error: "TSV must have a header row and at least one data row" },
        400,
        corsHeaders,
      );
    }
    const headerCells = lines[0].split("\t").map((c) => c.trim().toLowerCase());
    const idxKey = headerCells.findIndex((h) => h === "speaker_key" || h === "key");
    const idxName = headerCells.findIndex(
      (h) => h === "name" || h === "display_name",
    );
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const speakerKey = String(parts[idxKey] || "").trim();
      const displayName = String(parts[idxName] || "").trim();
      ...
      if (!displayName || !affiliation) {
        errors.push(`Line ${i + 1}: missing name or affiliation`);
        continue;
      }
      await upsertCatalogSpeakerRow(env, {
        speakerKey,
    ...
    return jsonResponse(
      {
        ...
        count,
        errors,
        message: `Upserted ${count} row(s).`,
      },
      200,
      corsHeaders,
    );
  } catch (e) {
    console.error("handleAdminImportSpeakerCatalogTsv:", e);
    return jsonResponse(
      {
        success: false,
        error: e?.message || "TSV import failed",
      },
      500,
      corsHeaders,
    );
